...
class WebcompatReporterDOMHandler : public content::WebUIMessageHandler {
 ...
 private:
  ...
  std::unique_ptr<brave::WebcompatReporterService> sender_;

  DISALLOW_COPY_AND_ASSIGN(WebcompatReporterDOMHandler);
};

WebcompatReporterDOMHandler::WebcompatReporterDOMHandler()
    : sender_(std::make_unique<brave::WebcompatReporterService>(
...
void WebcompatReporterDOMHandler::HandleSubmitReport(const base::ListValue* args) {
...
WebcompatReporterUI::WebcompatReporterUI(content::WebUI* web_ui, const std::string& name)
